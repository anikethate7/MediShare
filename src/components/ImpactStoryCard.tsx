...
export function ImpactStoryCard({ story }: ImpactStoryCardProps) {
  ...
  return (
    <Card className="flex flex-col h-full overflow-hidden rounded-xl shadow-xl hover:shadow-2xl transition-all duration-300 group bg-card border-2 border-transparent hover:border-accent/50">
      {story.imageUrl && (
        <div className="relative w-full h-40 sm:h-48 overflow-hidden rounded-t-xl"> {/* Adjusted height slightly */}
          <Image
      ...
      <CardHeader className="text-center items-center pt-5 pb-2 px-4">
         <div className={`bg-accent/10 p-3 rounded-full w-fit mb-3 group-hover:bg-accent/20 transition-colors ${story.imageUrl ? 'mt-0' : 'mt-1'}`}> {/* No negative margin if no image */}
          <BookOpen className="h-8 w-8 md:h-9 md:w-9 text-accent" /> {/* Adjusted icon size slightly */}
        </div>
