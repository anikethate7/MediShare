...
import { HeartPulse, Search, LogOut, LayoutDashboard, HandHeart, UserCircle, Building2, UserPlus2 } from 'lucide-react'; 
  ...
  const renderNavLinks = () => {
      ...
      return (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1 sm:gap-2">
          <Button variant="ghost" size="sm" asChild className="justify-start sm:justify-center">
            <Link href="/ngo-dashboard" className="flex items-center gap-1.5 text-sm">
    ...
    if (userRole === 'donor' && currentUser) {
      return (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1 sm:gap-2">
          <Button variant="ghost" size="sm" asChild className="justify-start sm:justify-center">
            <Link href="/donor-dashboard" className="flex items-center gap-1.5 text-sm">
              <LayoutDashboard className="h-4 w-4 text-accent" />
              My Dashboard
            </Link>
          </Button>
          <Button variant="ghost" size="sm" asChild className="justify-start sm:justify-center">
            <Link href="/browse-ngos" className="flex items-center gap-1.5 text-sm">
    ...
    return (
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1 sm:gap-2">
        <Button variant="ghost" size="sm" asChild className="justify-start sm:justify-center">
          <Link href="/donor" className="flex items-center gap-1.5 text-sm">
  ...
  return (
    <header className="bg-card shadow-md sticky top-0 z-50">
      <div className="container mx-auto px-4 py-3 flex flex-col sm:flex-row items-center justify-between">
        <Link href="/" className="flex items-center gap-2 text-primary hover:text-primary/80 transition-colors self-start sm:self-center mb-2 sm:mb-0">
          <HeartPulse className="h-8 w-8 text-accent" />
          <h1 className="text-xl md:text-2xl font-headline font-bold">MediShare</h1>
        </Link>
